...
from googleAdsDummy.engine.generators import (
    generate_campaign,
    generate_campaign_profiles,
)
...
class World:
    ...
    def _create_campaigns(self):
        ...
        self.campaign_profiles = generate_campaign_profiles(
            self.seed, campaigns_id, self.profile_rules
        )
    ...
    def is_date_valid(self, date_tobe_check: str) -> bool:
        ...
        return (
            self.date_period.start_date <= date_tobe_check <= self.date_period.end_date
        )

    def get_rules_snapshot(self) -> WorldRulesSnapshot:
        return WorldRulesSnapshot(
            seed=self.seed,
