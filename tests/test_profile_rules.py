...
def test_accepts_list_and_normalizes_upper():
    p = Profile_rules.model_validate([["a", "b", "a"], ["a"], {"b": 0.1}])

    assert p.allow_profiles == {"A", "B"}
    assert p.ensure_at_least_one == {"A"}
    assert p.distribution == {"B": 0.1}
...
def test_ensure_distributionSUm_dont_surpasses1():
    with pytest.raises(
        ValueError,
        match="distribution has poorly distributed values. The sum of all the values surpasses 1.0",
    ):
        Profile_rules.model_validate([["A", "B", "C"], ["A"], {"B": 0.5, "C": 0.6}])
