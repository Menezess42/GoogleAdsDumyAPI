...
def print_query_tree(query, use_colors: bool = True):
    ...
    def print_node(node: Any, prefix: str = "", is_last: bool = True, label: str = ""):
        ...
        if label:
            print(f"{prefix}{connector}{BOLD}{CYAN}{label}:{RESET} {BLUE}{node_name}{RESET}")
        else:
        ...
        if node_name == "QueryNode":
            ...
            print_node(node.from_clause, new_prefix, node.where is None and node.order_by is None and node.limit is None, "FROM")
            
            if node.where:
                print_node(node.where, new_prefix, node.order_by is None and node.limit is None, "WHERE")
            if node.order_by:
        ...
        # FieldNode
            ...
            print(f"{new_prefix}├── {GREEN}resource:{RESET} {format_value(node.resource)}")
        ...
        # ComparisonNode
            ...
            print(f"{new_prefix}├── {GREEN}operator:{RESET} {YELLOW}{node.operator}{RESET}")
        ...
        # OrderByItemNode
            ...
            print_node(node.field, new_prefix, False, "field")
            print(f"{new_prefix}└── {GREEN}direction:{RESET} {MAGENTA}{node.direction}{RESET}")
